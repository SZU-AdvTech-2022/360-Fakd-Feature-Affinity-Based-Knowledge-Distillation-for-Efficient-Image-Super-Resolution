...
import utility
import math
from feature_transformation import spatial_similarity, channel_similarity, batch_similarity, FSP, AT, SP, CC, FT, testdis
...
def train(epoch):
    ...
    for batch, (lr, hr, _, idx_scale) in enumerate(train_loader):
        ...
        if args.feature_loss_used == 1:	
            ...
            if 'SP' in args.feature_distilation_type:
                aggregated_student_fms.append([SP(fm) for fm in student_fms])
                aggregated_teacher_fms.append([SP(fm) for fm in teacher_fms])
            if 'CC' in args.feature_distilation_type:
                aggregated_student_fms.append([CC(student_fms[i], student_fms[i+1]) for i in range(len(student_fms) - 1)])
                aggregated_teacher_fms.append([CC(teacher_fms[i], teacher_fms[i+1]) for i in range(len(teacher_fms) - 1)])
            if 'FT' in args.feature_distilation_type:
                aggregated_student_fms.append([FT(fm) for fm in student_fms])
                aggregated_teacher_fms.append([FT(fm) for fm in teacher_fms])
            if 'test' in args.feature_distilation_type:
                aggregated_student_fms.append([testdis(fm) for fm in student_fms])
                aggregated_teacher_fms.append([testdis(fm) for fm in teacher_fms])
        ...
        for idx_data, d in enumerate(test_loader):
            for idx_scale, scale in enumerate(args.scale):
                ...
                for lr, hr, filename, _ in tqdm(d, ncols=80):
                    ...
                    ssim = utility.get_ssim(sr, hr, scale, args.rgb_range, benchmark=d)
                    if args.save_gt:
